...
int iterMandelbrot( int maxIter, const complex& c)
{
	...
	complex ct{c.re-0.25,c.im};
        double ctnrm2 = std::sqrt(ct.re*ct.re+ct.im*ct.im);
        if (ctnrm2 < 0.5*(1-ct.re/ctnrm2)) return maxIter;
    }
    int niter = 0;
    while ((z.re*z.re+z.im*z.im< 4.) && (niter < maxIter))
    {
...
/*    int nbtasks, rank;
    ...
    MPI_Finalize();
**/    
    savePicture("mandelbrot_ref.png", W, H, iters, maxIter);
    return EXIT_SUCCESS;
 }
