...
template<typename K> variable_<K> variable(K& var) { return {var}; }
...
struct norm_tag{};
template<typename Expr> struct norm_ : public unary_op_<Expr,norm_tag>
{
    norm_( const Expr& e ) : unary_op_<Expr,norm_tag>(e) {}
};
template<typename Expr> norm_<typename expr_traits<Expr>::type> norm(const Expr& e) {
    return {typename expr_traits<Expr>::type(e)};
}

struct sqr_tag{};
template<typename Expr> struct sqr_ : public unary_op_<Expr,sqr_tag>
{
    sqr_( const Expr& e ) : unary_op_<Expr,sqr_tag>(e) {}
};
template<typename Expr> sqr_<typename expr_traits<Expr>::type> sqr(const Expr& e) {
    return {typename expr_traits<Expr>::type(e)};
}
